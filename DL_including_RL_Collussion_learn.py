...
if __name__ == '__main__':
    ...
    NOW ='Collussion_learning' + datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
    ...
    Test_process('Test_data/Test_data.json', RL_SAVE_PATH)
    ...
    DL_process(DL_database)
    FM.Network_copy('relative_network', True)
    print('start TEST',datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
    Test_process('Test_data/Test_data.json', TEST_SAVE_PATH)
    Transform_data_to_relative_coordinate(TEST_SAVE_PATH +'/TEST_Path.json', DL_database)
